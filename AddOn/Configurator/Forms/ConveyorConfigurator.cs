...
namespace B1C.SAP.Addons.Configurator.Forms
{
    ...
    public class ConveyorConfigurator : Window.FortressConfiguration
    {
        #region Constuctors
        /// <summary>
        /// Initializes a new instance of the <see cref="ConveyorConfigurator"/> class.
        /// </summary>
        /// <param name="addOn">The add on.</param>
        /// <param name="xmlPath">The XML path.</param>
        /// <param name="formType">Type of the form.</param>
        public ConveyorConfigurator(UI.AddOn addOn, string xmlPath, string formType) : base(addOn, xmlPath, formType) { }
        #endregion
        ...
        {
            get { return ConfigurationType.ConveyorBelt; }
        }
        ...
        private void OnAfterArgumentsSet(object sender)
        {
            ...
            else
            {
                ...
                using (var rs = new RecordsetAdapter(this.AddOn.Company, string.Format("select Code from [@XX_CONVBLT] WHERE U_XX_OrderNo = '{0}' AND U_XX_OrdrLnNo = {1}", this.OrderId, this.OrderLine)))
                {
                    if (!rs.EoF)
                    {
        ...
        private void OnAfterFormLoad(object sender, FormEventArgs e)
        {
            try
            {
                var folder = (SAPbouiCOM.Folder)this.ControlManager.Item("tabCnvMn").BaseItem.Specific;
                folder.Select();
                folder = (SAPbouiCOM.Folder)this.ControlManager.Item("tabCnvMst").BaseItem.Specific;
                folder.Select();
            }
            catch (Exception ex)
            {
        ...
        private void OnFolderClick(object sender, ItemEventArgs e)
        {
            ...
            switch (e.ItemId)
            {
                case "tabCnvMn":
                    ...
                    break;
                case "tabRejMn":
                    folder = (SAPbouiCOM.Folder)this.ControlManager.Item("tabRejMst").BaseItem.Specific;
                    folder.Select();
                    break;
            }
        }
